...
A separate line of work evaluates the detectors themselves. TRUE \citep{honovich-etal-2022} standardises factual consistency evaluation over eleven annotated datasets; AGGREFACT \citep{tang-etal-2023} aggregates nine summarisation datasets and reports that no single metric is best in all settings or for all error types; RAGTruth \citep{niu-etal-2024} supplies word-level annotations over roughly 18,000 retrieval-augmented responses; FaithBench \citep{bao-etal-2025} is assembled from summaries on which current detectors disagree and reports detection near 50\%; MiniCheck \citep{tang-etal-2024} unifies recent grounding datasets and trains compact fact-checkers against them.

These compare detectors against gold hallucination labels and report detector accuracy. Our design contains no gold hallucination label: we hold the grid, the queries and the answers fixed, swap the evaluator, and ask whether the ranking of the systems under study survives. The two questions come apart --- a detector more accurate on a labelled corpus need not resolve differences between the systems a practitioner is choosing between, and it is that second property which decides whether a published comparison describes the systems or the instrument. \citet{xiao-etal-2023} argue generally that generation metrics should be analysed as measurement instruments with stated reliability and validity; Sections~\ref{sec:sensitivity} and \ref{sec:equivalence} are two such analyses, on the instruments a RAG faithfulness result is usually reported with.
...
An earlier version of this work differenced NDCG@5 and faithfulness into a gap metric, with a normalised variant dividing by retrieval quality. We retire it, for reasons general to that family: the two terms have no common unit; the difference is negative for every model on Natural Questions, since faithfulness (0.85) exceeds retrieval quality (0.79), so the metric presumes a loss of fidelity that does not occur; any metric of the form 1 - F/RQ reproduces the retrieval ranking whenever F is near-constant, making its agreement across generators uninformative; and a difference cannot separate a system whose retrieval and faithfulness are both high from one where both are low. We report absolute faithfulness throughout, which keeps the both-low case visible.
...
Cases are taken in dataset order rather than at random --- the first 200 eligible answers for each system, dataset and generator --- so the scored set is a prefix of an already filtered population: 4,784 cases against 7,609 eligible, ranging from a near-census on GPT-4o-mini (800 of 882 on NQ) to two fifths on Claude (800 of 1,946).

Two controls accompany it: an entity substitution, giving a comparable edit on a named entity rather than a value, and the untouched answer scored against another query's retrieved context, which gives the floor of each evaluator's scale on this data. Cases are rebuilt deterministically so all three evaluators see identical inputs.
...
Detection rates are comparable across evaluators only after the scale is normalised, since the evaluators do not share a floor, so we report two gates. On Claude's NQ cell an untouched answer scored against an unrelated context averages 0.474 under NLI-max against 0.245 under claim-min, and that distribution is heavy-tailed --- its 95th percentile is 0.985 under NLI-max --- so a fixed 0.5 gate sits barely above the floor for one evaluator and an unrelated document clears it nearly half the time. The second gate is anchored at that 95th percentile, where "detected" means the falsified answer is no longer better supported than an unrelated document: the same statement on every scale. Read that way NLI-max detects 50.2\% of falsifications there rather than 5.1\%, and its gap to GPT-4o-mini narrows from roughly elevenfold to under twofold. The evaluator ordering is unchanged at either gate. The floor-anchored gate is a comparison device, not an operating point --- it sits above the mean untouched answer, so no deployed system could use it.


\subsection{The effect is not verbatim copying}
\label{sec:copying}

A natural explanation is that one generator quotes the retrieved text back, so the evaluator scores a copy of its own premise and a single substituted value cannot move it. The data does not support it. Over the 4,784 cases, the fraction of answer word 5-grams occurring in the retrieved context as a contiguous token sequence is 0.226 for Claude, 0.194 for Qwen and 0.235 for GPT-4o-mini: the explanation requires overlap to run opposite to sensitivity, and instead the least sensitive generator does not copy the most and the one that copies least is the intermediate one. The correlation between overlap and the response to falsification is +0.046 for Claude (95\% CI -0.004 to +0.096), +0.178 for Qwen (+0.127 to +0.229) and +0.166 for GPT-4o-mini (+0.118 to +0.212) --- indistinguishable from zero in the first case and positive in the other two, where the explanation requires all three to be negative.
...
The three generators average 3.08, 1.77 and 1.25 assertions per attempted answer on this dataset, with median answer lengths of 55, 25 and 15 words. This largely accounts for the cross-generator difference in Table~\ref{tab:2}: a falsified value is a progressively smaller edit to the hypothesis as correct surrounding text accumulates, and the entailment judgement is dominated by the remainder. It does not account for it entirely --- within the single-assertion bucket the three generators still differ, +0.102 against +0.409 and +0.485. As a check on the claim-level implementation, every single-assertion answer scores identically under both aggregates (n = 2,472, largest absolute difference 1.7e-05, which is batch-padding noise), the expected degenerate case. That check is what surfaced the normalisation defect reported in Section~\ref{sec:normalisation}: before markdown was stripped for both aggregates it failed on every one of the 131 formatted answers, by as much as 0.729.
...
All three generators decline to answer on a substantial share of queries, and the rate depends on retrieval quality: on HotpotQA the weakest retriever draws 0.378 to 0.398 refusals and the strongest 0.224 to 0.241. A refusal is not an attempt at a grounded answer, so pooling refusals into a faithfulness average transfers a retrieval effect into it at no cost in plausibility.
...
The transfer is large --- a pooled spread of 0.053 to 0.085, against 0.003 to 0.029 once refusals are excluded --- but its direction is not a property of refusing. It is a property of how the evaluator scores the wording each generator refuses in. Claude's refusals score far below its attempts, so pooling drags the weakest retriever down and the pooled column ascends with NDCG@5. Qwen's and GPT-4o-mini's score far above theirs, so pooling lifts the weakest retriever and the same column descends. Both readings come out of one experiment and neither is about faithfulness: the pooled column is close to a monotone function of the refusal rate, with a sign set by one sentence of boilerplate. A defender of the pooled statistic cannot fall back on a consistent direction of bias, because the direction reverses across generators on identical queries and identical retrieval. All faithfulness results in this paper exclude refusals and are paired on the queries every system answered.


\subsection{Membership Is Set by a Grader, and Graders Disagree}
\label{sec:graders}

Excluding refusals requires deciding which answers are refusals, and a rule makes that decision. Two defensible rules --- a string match against the refusal templates the prompt offers, and a judge model shown the question, the reference answers and the answer --- agree on the large majority of rows and part at the margin, the judge finding four to five more refusals per hundred in every system without reordering them. The judge was run on the two proprietary arms only, so this subsection and the next cover sixteen of the twenty-four cells.

A level shift that preserves the ordering would ordinarily be unremarkable. It is not, because the rows it moves are not a random sample of the population: they are the ones nearest the boundary between refusing and attempting. Substituting one detector for the other moves nineteen rows on NQ/Claude and fifty-five on HotpotQA/Claude, and carries the uncorrected p-value across 0.05 in two of the eight cells it was run over --- NQ/Claude under the entailment aggregate from 0.223 to 0.048, HotpotQA/Claude under AlignScore from 0.151 to 0.049. Both land within a thousandth of the threshold, which is the point: the verdict in those cells is not a property of the systems. Neither survives correction for multiplicity, and the number of cells whose verdict changes between evaluators is the same under either detector --- but they are not the same cells. We report the count as the finding and treat any individual cell as undetermined.


\subsection{The Correctness Grader Admits Refusals to the Wrong Population}
\label{sec:refusals}

The same problem reappears once faithfulness is conditioned on correctness. String containment grades an answer correct when a reference answer string occurs in it, and a short reference string can occur inside a refusal; on a 200-answer calibration ten refusals were graded correct this way and the judge overturned nine. Such a row carries a near-floor faithfulness score into the correct-answer average and lowers the baseline any correct-versus-incorrect comparison is measured against. Under graded judgement no refusal is graded correct in any of the sixteen judged cells.

The effect is large enough to change a sign. With containment, and a baseline that excluded refusals from the incorrect side only, answers on NQ/Claude appeared more grounded when wrong than when right by 0.018 to 0.050. Excluding refusals from both sides reduces that to -0.006 to +0.018; grading with the judge instead of containment moves it to -0.051 to -0.113, the other way. Each step corrects a grader rather than changing an answer, and all three move the quantity the same way. Tested directly, no positive value survives: over the sixteen judged cells a two-sample permutation test with Holm correction finds three significant, all negative, and no positive-signed cell has an interval excluding zero. We report no evidence that answers are more grounded when they are wrong. The intervals are 0.08 to 0.13 wide, because 44 to 84 wrong answered rows remain per cell, so these tests bear on the general claim and not on individual systems.
...
The same accounting applies to the contingency between retrieval success and correctness. The cell in which retrieval succeeded and the answer was still wrong is the one that bears on whether retrieval quality is sufficient, and most of it was never attempted. Read without that decomposition it would support the claim that good retrieval frequently fails to produce a correct answer; most of what it counts is a system declining to answer, and refusal rates differ by more than twenty points across the systems compared. The residual is real and roughly a fifth the size the raw cell implies.
...
The statistic is the spread: the difference in mean faithfulness between the best and worst of the four systems, with the p-value from a paired bootstrap over that pair, re-centred at zero \citep{dror-etal-2018}. It is the quantity a practitioner is exposed to, since they adopt one embedding model and stand to lose the distance to the best alternative rather than the dispersion of the set. But it summarises two systems selected for being the extremes, so its p-value is not adjusted for that within-cell selection and the two intermediate systems do not enter it. We therefore treat it as descriptive and report the direction across all four systems separately in Table~\ref{tab:8}. An omnibus test would be the stricter statistic; we do not run one.
...
Adding a third generator made the correction stricter --- eighteen comparisons rather than twelve --- and the count rose anyway, which the design does not guarantee. It also produced the sharpest result here: under AlignScore on HotpotQA all three generators rank the four systems identically (all-mpnet $<$ text-embedding-3-small $<$ BGE-M3 $<$ E5-instruct), while under NLI-max no two agree. Read this carefully. The generators are independent but answer from the \emph{same} retrieved contexts, so it is three generators agreeing about a property of the shared retrieval, not three independent replications. What it establishes is that one evaluator resolves that property consistently across generators and another does not, on identical inputs.

That count of two is not invariant to the correction either. Two comparisons sit either side of alpha = 0.05 depending on which are treated as one family: NQ/Claude under AlignScore is p = 0.0081 uncorrected, adjusted to 0.122 over the eighteen in the table, 0.032 over the six sharing an evaluator and 0.024 over the three in its own cell; HotpotQA/GPT-4o-mini under claim-min is p = 0.0070, adjusted to 0.112, 0.042 and 0.014. Either smaller family makes the count four. Correcting within evaluator is defensible --- the six NLI tests establish which cells are null, so they precede the question rather than competing with it --- but we report the eighteen-comparison family because it is the most conservative and was fixed before the correction was applied; adopting a smaller one after seeing that it restores cells would be a selection we could not account for. We state the alternative rather than only the choice, because a count that moves with the multiplicity family is the same dependence on an analysis decision that this paper reports for the choice of evaluator.
...
An earlier version of this analysis, before the open-weight generator was added, reported two of the four cells then in the design rather than one. The difference was not a change of test, data or population but of text normalisation --- an instance of the paper's own subject.

The entailment scorer read the answer as the generator wrote it while the claim-level scorer stripped markdown first, so the two aggregates were not reading the same hypothesis. That discrepancy is invisible in every summary statistic and survives every significance test. It was caught by the degenerate-case check in Section~\ref{sec:assertions}, an identity the two aggregates must satisfy exactly, which failed on all 131 formatted answers then in the grid, by up to 0.729 on a pair of asterisks. Markdown is not evenly distributed --- 76.0\% of Claude's attempted answers against 1.3\% and 0.3\% for the other two --- so one generator's formatting habits were entering a comparison between embedding systems. Stripping it for all three restores the identity and removed the significance of NQ/Claude under AlignScore, the cell Table~\ref{tab:7} still places just outside the threshold. Neither cell that survives correction is affected; both are on generators that emit almost no markdown. Metrics that admit such an identity should be checked against it.


\section{What an Equivalence Claim Requires}
\label{sec:equivalence}

Four of the six cells in Table~\ref{tab:7} show no detectable difference under any of the three evaluators. Absence of a detected difference is not evidence of equivalence, so we test equivalence directly with two one-sided tests \citep{schuirmann-1987}. A TOST result depends entirely on the margin, and a margin is a substantive judgement rather than a statistical fact --- so we report a curve rather than assert a point.
...
For a document question answering system, the practical reading of Table~\ref{tab:2} is that a grounding threshold on a verbose generator catches a minority of falsified values at a fixed gate: 5.1\% under the common evaluator, 25.1\% under the most responsive one tested. A deterministic check that every value in an answer appears in the retrieved context addresses this class of error directly. Run over the same cases, it recalls every falsified value in all six cells at a false-positive rate of 0.8\% to 5.2\%. Two qualifications matter. Numbers that assert nothing about the world --- the chunk indices the prompt supplies, the markers of an ordered list --- are not expected in the retrieved text, and counting them raises that rate on Claude's HotpotQA answers to about 39\%. And the residual false positives are quantities the answer derives rather than copies: counts, sums, elapsed years, converted formats. A literal check flags all of those, so it complements an entailment score rather than replacing one.

Checking symbolic content against the source is not a new proposal and we do not claim it as one: \citet{goodrich-etal-2019} compare extracted subject-relation-object triples against the source, \citet{nan-etal-2021} define entity-level consistency metrics for summarisation, and \citet{zhao-etal-2020} verify dates, numbers and monetary amounts directly. That numbers are a weak point is documented at the level of the representations too --- \citet{wallace-etal-2019} find magnitude encoded unevenly across standard embeddings and least reliably by subword models. Entailment is separately a loose proxy for attribution: the AIS framework \citep{rashkin-etal-2023} separates support by an identified source from general plausibility, which is the distinction a structurally similar but differently attributed claim exploits.

What this paper adds is narrower than the check itself: the controlled minimal pair. One numeric value is replaced in an otherwise byte-identical generation and the same case is scored by all three evaluators, so what is measured is each evaluator's response to a known error on identical input, not its agreement with a human label on some other corpus. That measurement covers numeric values, three generators and two datasets.
