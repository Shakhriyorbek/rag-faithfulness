...
That is enough to change a sign. With containment, and a baseline excluding refusals from the incorrect side only, NQ/Claude answers looked more grounded when wrong than when right by 0.018 to 0.050. Excluding refusals from both sides gives -0.006 to +0.018; grading with the judge gives -0.051 to -0.113. Each step corrects a grader rather than an answer, and all three move the quantity the same way. Tested directly over all twenty-four judged cells, a permutation test with Holm correction finds six significant, every one of them negative, and none of the four positive-signed cells has an interval excluding zero. E5-instruct on Natural Questions is negative under all three generators (-0.105, -0.115, -0.125) and survives correction in each. We report no evidence that answers are more grounded when wrong. Intervals are 0.064 to 0.129 wide on 44 to 134 wrong answered rows per cell, so this bears on the general claim rather than on individual systems.
...
\item The three evaluators receive byte-identical answers but not identical premises. The entailment scorer is given each of the five retrieved chunks separately, plus their concatenation truncated to the model's window, and the maximum is taken; the claim-level scorer applies that construction per extracted claim; AlignScore is given the concatenated context as one premise and performs its own splitting. Each is the evaluator used as its authors intend, but it means part of the difference reported in Section~\ref{sec:sensitivity} is premise granularity rather than evaluator sensitivity. Re-scoring the same cases with the same entailment model on the concatenated premise bounds that share. Expressed as a fraction of the NLI-max-to-AlignScore gap it runs from -0.55 to +0.20 across the six cells, and is negative in four of them: the concatenated premise makes the entailment model \emph{less} responsive to a falsified value, not more. Premise granularity therefore does not account for AlignScore's advantage, since in most cells it works against it. The hypothesis, by contrast, is now normalised identically for all three (Section~\ref{sec:normalisation}); it was not in an earlier version, and that difference alone decided one cell.
