...
ReDeEP \citep{sun-etal-2025} localises hallucination to attention heads and feed-forward components. \citet{sinha-2025} reports that embedding-based detection separates synthetic hallucinations well but real ones poorly, the hard cases being those that stay semantically close to a faithful response. \citet{chen-etal-2025} vary and then combine embedding models for mathematics question answering, having observed that different embedders succeed on different queries. We do not probe model internals. What we report here is behavioural.
...
Four further arms run off the same grid and are reported in Sections~\ref{sec:control} to~\ref{sec:qasper}. A fifth embedding model, jina-embeddings-v3, was run over both datasets and all three generators and serves as a control on the comparison rather than a member of it. A third dataset, QASPER \citep{dasigi-etal-2021}, was run over all five embedding models and all three generators at 819 questions. Floor, ceiling and context-arrangement conditions, and the geometric and re-ranking analyses, were run on the Claude arm; their scope is stated where they appear and in the Limitations.
...
Under the common evaluator no cell of the six shows a detectable difference; under the evaluator Section~\ref{sec:sensitivity} measures as most responsive to known errors, two do. A result reported only under the first would describe the resolution of the instrument, not a property of the systems. Both surviving cells are on HotpotQA, which has the wider retrieval spread, and neither is on Claude, whose answers the entailment aggregate resolves least well.
...
Four of the six cells are null, which invites the question of whether the test can detect anything at all. It can, and the same grid supplies the control. jina-embeddings-v3 \citep{sturua-etal-2024} was indexed and generated over on both datasets under all three generators, and on Natural Questions it retrieves considerably worse than the four systems above: NDCG@5 of 0.685 against a band of 0.786 to 0.830. It widens the spread the comparison is run on from 4.4 points to 14.5. Paired TOST on per-query NDCG puts its mean difference from the other four at 0.102 to 0.145, with $p_{\mathrm{TOST}} = 1.0000$ against each --- the furthest from equivalence the test can report. On HotpotQA it sits inside the existing band instead, and its pair with text-embedding-3-small is the only one anywhere in our grid that is both undifferentiated and TOST-equivalent at $\pm$0.02 (mean difference 0.0059, $p_{\mathrm{TOST}} = 0.0083$).

Rerunning Table~\ref{tab:7} over five systems rather than four, with the same test, the same resampling and the same correction, takes the count from two of six cells to \emph{six of six}. Every cell differs, under every evaluator.

We do not read that as a stronger result. It is the precondition failing. The comparison in this section is between systems that retrieve about equally well and are nonetheless scored differently, and a system that retrieves worse should be scored differently, by any evaluator worth using. What the control establishes is that the four nulls are not a power failure: at these sample sizes the procedure separates systems that genuinely differ, in every cell, and it declines to separate the matched four. A null obtained from an instrument that never fires would carry no information. This one is obtained from an instrument that fires whenever it is given a real difference.

It also bounds how far the paper's own framing can be pushed. Matched retrieval quality is not a property of a convenient subset of embedding models; it is a condition that has to be checked, and one of the five models in this repository fails it on one of the two datasets.
...
Everything above compares retrieval systems to each other. Two anchors say what the comparison is inside of. Both were run on the Claude arm only, at $n = 1000$ per dataset.

\textbf{The floor.} With no retrieved context the generator answers 32.2\% of Natural Questions correctly, declining 15.5\% of the time. Both benchmarks are built on Wikipedia, and roughly a third of the single-hop result is therefore reachable without retrieval at all. On HotpotQA the same condition gives 20.9\% correct and a 70.4\% refusal rate: asked to compose across documents it does not have, the model mostly declines rather than guessing. Any claim that retrieval caused a correct answer on Natural Questions has this floor underneath it.

\textbf{The ceiling.} Supplying exactly the answer-bearing chunks gives 77.0\% on Natural Questions and 86.8\% on HotpotQA. On HotpotQA that behaves as a ceiling: the best of the four systems reaches 81.0\% of it and the worst 60.8\%. On Natural Questions three of the four systems \emph{exceed} it, at 102.2\%, 102.6\% and 103.4\%.

We report that rather than clipping it, because the explanation is a design choice worth stating. The oracle condition supplies the answer-bearing chunks and nothing else, which is narrower context than top-five retrieval, and a narrower context produces more refusals on questions that need surrounding material --- 12.9\% here against a floor of retrieval's own. It is a ceiling on evidence \emph{selection}, not on accuracy, and on a single-hop dataset the extra material retrieval brings is worth more than the extra noise costs. On a multi-hop dataset, where the composition is the difficulty, it is not.

\textbf{Arrangement.} Over 300 queries per condition we vary how the gold evidence is presented (Table~\ref{tab:ablation}). Order does not matter: reversing or shuffling it moves accuracy by at most 1.7 points on either dataset, and holding length at five chunks while moving a single gold chunk between first, middle and last position spans 0.727 to 0.757 on Natural Questions and 0.503 to 0.530 on HotpotQA. At $k = 5$ we see no position effect.

Subset does matter, and only where composition does. Reducing the context to one gold chunk costs Natural Questions two points and HotpotQA thirty-six, and the refusal column says why: HotpotQA refusals rise from 7.3\% to 75.7\%. Given half of a multi-hop chain the model declines rather than completing it from memory. That is the behaviour one would want, and it is produced by the same refusal machinery that Section~\ref{sec:population} shows will distort a faithfulness average if those answers are left in it. Retrieval failure and refusal are not independent events, and an evaluation that averages over both is measuring their sum.
...
\hline
\end{tabular}
\caption{Accuracy and refusal rate when the gold evidence is rearranged, $n = 300$ per condition, Claude. The last four conditions hold context length at five chunks. Order is inert; losing half a multi-hop chain is not, and it converts into refusals rather than errors.}
\label{tab:ablation}
\end{table}

The distractor-only condition gives a second floor and it is not zero: 9.0\% and 15.0\% correct from hard negatives alone, consistent with the no-retrieval result above.


\section{Geometric Analysis and Re-ranking}
\label{sec:esa}

A natural response to any of this is to re-rank retrieved chunks by how well they support an answer. That presumes a usable signal at retrieval time, which is testable and turns out to be dataset-dependent.

The quantity that motivates re-ranking is the correlation between a chunk's similarity to the query and its entailment of the answer. There are two versions and they are not interchangeable. Correlating with entailment of the \emph{gold} answer measures a static property of the embedding space; correlating with entailment of the \emph{query} measures the signal actually available before any answer exists. We compute both, $n = 200$ per system and dataset.

On Natural Questions both are positive and agree closely: gold-answer correlations of 0.146 to 0.217 and query correlations of 0.131 to 0.226, significant at $\alpha = 0.05$ in three of four systems for each. On HotpotQA the gold-answer correlation is \emph{negative} in all four systems ($-0.086$ to $-0.115$) and the query correlation is indistinguishable from zero (0.037 to 0.077, no $p$ below 0.27). The geometry carries a weak signal on single-hop retrieval and none on multi-hop.

Re-ranking then does what that predicts. Taking the top twenty for one system and re-ranking by entailment of the query at $\lambda = 0.6$ before regenerating, faithfulness on attempted answers moves by $+0.008$ on Natural Questions (95\% CI $[-0.007, +0.023]$) and $-0.008$ on HotpotQA ($[-0.029, +0.012]$). Both intervals contain zero. What does move is the refusal rate, from 15.4\% to 19.0\% on Natural Questions: re-ranking removes context the generator had been using, and it declines rather than answering from less.

Faithfulness-aware re-ranking is often proposed on the strength of a correlation like the one above. This is what that correlation is worth when it is spent: under one point of faithfulness in either direction, and neither interval excludes zero. A correlation of 0.13 to 0.23 on the one dataset where it exists is not enough to convert into a gain, and on the other there is nothing to convert.


\section{When Retrieval Collapses}
\label{sec:qasper}

The two benchmarks above are Wikipedia, and both are ones these retrieval systems do well on. QASPER \citep{dasigi-etal-2021} is neither. Running the same grid over 819 questions on NLP papers, with all five embedding systems, retrieval essentially fails: NDCG@5 of 0.062 to 0.122 against 0.705 to 0.830 on the other two, and Recall@5 between 8\% and 18\%. The cause is structural rather than a property of the retrievers. QASPER is single-document question answering, and pooling 819 papers into one corpus of 13,876 chunks leaves questions like ``which multilingual approaches do they compare with?'' carrying no signal that identifies which paper they belong to.

Refusal rates rise accordingly, to 27.8\%, 35.4\% and 45.0\% for the three generators, but between 2,254 and 2,955 answers per generator remain. On those, two orderings from the main grid invert.

The generators swap places. GPT-4o-mini is the most faithful here under all three evaluators and Claude the least, the reverse of Section~\ref{sec:verdict}. More consequentially, so do the evaluators. Across all six cells of the main grid the three evaluators order identically, AlignScore above claim-min above NLI-max. On QASPER that holds only for GPT-4o-mini. For Claude the order reverses to NLI-max 0.712, AlignScore 0.629, claim-min 0.240.

The 0.240 is the informative number. Claim-min reports the least supported assertion in an answer, so a value that low means Claude is producing multi-assertion answers whose weakest component the retrieved context does not support --- which is what one would expect when the context usually lacks the evidence and the generator answers anyway. NLI-max, taking the maximum over chunks, still finds something to entail and reports 0.712 on the same answers.

We do not fold this into the main grid, because a population where retrieval failed cannot test a claim about systems at matched retrieval quality. What it adds is a boundary condition on Section~\ref{sec:verdict}. The relative ordering of these evaluators is not a fixed property of the evaluators. It is a property of the evaluators applied to a particular retrieval regime, and it inverts when that regime does.
...
For a document question answering system, the practical reading of Table~\ref{tab:2} is that a grounding threshold on a verbose generator catches a minority of falsified values: 5.1\% under the common evaluator, 25.1\% under the most responsive one tested. A deterministic check that every value in an answer appears in the retrieved context addresses this directly, recalling every falsified value in all six cells at a false-positive rate of 0.8\% to 5.2\%. Two qualifications matter. Numbers that assert nothing about the world, such as chunk indices and ordered-list markers, are not expected in the retrieved text, and counting them raises that rate on Claude's HotpotQA answers to about 39\%. The rest are quantities the answer derives instead of copying, which a literal check also flags, so it complements an entailment score rather than replacing one.
...
\item The repository configures seven embedding models. Four carry the matched comparison --- all-mpnet-base-v2 (contrastive), BGE-M3 (multilingual), E5-large-instruct (instruction-tuned) and text-embedding-3-small (proprietary) --- chosen to span pretraining paradigms and to include one closed-source arm. A fifth, jina-embeddings-v3 \citep{sturua-etal-2024}, was run over both datasets and all three generators and is reported in Section~\ref{sec:control} as a control rather than as a member of the matched set, because it fails the matched-quality precondition on Natural Questions. GTE-large and Instructor-XL remain unrun: each additional model is a further generation pass over every dataset and generator, and the grid is a testbed for the evaluator comparison rather than a survey of embedding models.

\item The floor, ceiling and context-arrangement conditions of Section~\ref{sec:anchors} were run on the Claude arm only, and the geometric analysis and re-ranking of Section~\ref{sec:esa} on one embedding system. Neither set of conclusions has been shown to hold across generators, and the re-ranking result in particular is a single operating point ($\lambda = 0.6$) on a single system, so it bounds what that intervention achieved here rather than what such interventions can achieve.

\item The oracle condition supplies answer-bearing chunks only, which is narrower context than top-five retrieval. It is therefore a ceiling on evidence selection and not on accuracy, which is why retrieval exceeds it on Natural Questions. A ceiling defined as gold passages in full would be higher and less comparable to what retrieval produces.
