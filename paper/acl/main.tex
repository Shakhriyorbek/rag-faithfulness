...
\begin{table}[t]
\centering\small
\begin{tabular}{p{2.85cm}ccc>{\centering\arraybackslash}p{1.15cm}}
\toprule
 & \textbf{NLI} & \textbf{Claim} & \textbf{Align} & \textbf{Value chk.} \\
\midrule
Detects a falsified value, terse generator & 57--71\% & 61--71\% & 75--89\% & 100\% \\
\addlinespace
\dots verbose generator & 5--14\% & 25--34\% & 24--49\% & 100\% \\
\addlinespace
Sensitivity to answer length (gap above) & 54 pts & \textbf{36 pts} & 46 pts & none \\
\addlinespace
Resolving power, MDE at 80\% & .021--.060 & .024--.050 & .012--.037 & --- \\
\addlinespace
Scores a refusal above a real answer? & 2 of 3 gens & 2 of 3 gens & never & --- \\
\addlinespace
Error types covered & any & any & any & symbolic \\
\addlinespace
False positives & --- & --- & --- & 0.8--5.2\% \\
\bottomrule
\end{tabular}
\caption{What we measured about each instrument, arranged for choosing one. Detection is against a falsified numeric value at the fixed gate (Table~\ref{tab:2}); the entity probe gives the same ordering (Table~\ref{tab:entity}). MDE is from Appendix~\ref{app:power}. Refusal behaviour is the mean refusal score against the mean attempted-answer score, per generator. The sensitivity row is the detection gap between the two generators, averaged over the two datasets; claim-min is the least length-sensitive of the three and still gives up 36 points. The deterministic value check covers only symbolic content, which is why it supplements an evaluator instead of replacing one.}
\label{tab:choose}
\end{table}
